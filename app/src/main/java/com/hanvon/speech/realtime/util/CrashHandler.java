...
//            mDefaultHandler.uncaughtException(thread, ex);
    ...
    private boolean handleException(Throwable ex) {
        ...
        sb.append(err);
        String path = ConstBroadStr.GetAudioRootPath(mContext,false) + "/crash/";
       /* File f = new File(path);
        if (!f.exists())
            f.mkdirs();*/
        hvFileCommonUtils.createDirIfNeed(mContext, path);
        Log.e(path, "path：" + path);
        String temp = path + "crash" + ".txt";
        File file = new File(temp);
        hvFileCommonUtils.createFileIfNeed(mContext, file);
        hvFileCommonUtils.WriteToFile(mContext, temp, sb.toString());
        /*try {
